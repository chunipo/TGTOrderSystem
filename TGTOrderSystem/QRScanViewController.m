...
- (void)getResponse:(NSString *)qrCodeString {
    ...
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:nil message:[NSString stringWithFormat:@"扫描结果：%@", qrCodeString] preferredStyle:UIAlertControllerStyleAlert];
    [alert addAction:[UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
        [_scanView startScanQrCode];
    }]];
    [alert addAction:[UIAlertAction actionWithTitle:@"查询该订单" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        if (self.delegate != nil && [self.delegate respondsToSelector:@selector(getQrCodeResponse:)]) {
...
- (ScanView *)scanView {
    if (!_scanView) {
        _scanView = [[ScanView alloc] initWithFrame:CGRectMake(0, 0, KScreenWidth, KScreenHeight)];
